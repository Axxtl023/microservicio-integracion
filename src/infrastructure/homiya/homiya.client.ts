...
export class HomiyaClient implements IHomiyaClient {
  ...
  constructor() {
    this.http = axios.create({
      baseURL: process.env.HOMIYA_API_URL ?? 'https://apigateway-mr.onrender.com',
      timeout: 15_000,
  ...
  async verificarDisponibilidadHotel(alojamientoId: string, habitacionId: string): Promise<DisponibilidadHotelDto> {
    try {
      ...
      return {
        ...
        mensaje: exists ? 'Habitación disponible en catálogo' : 'Habitación no encontrada en Homiya',
      };
    } catch (err) {
      this.logger.error(`[Homiya] Error verificando disponibilidad para alojamientoId=${alojamientoId} habitacionId=${habitacionId}`, err);
      return {
        alojamientoId,
        habitacionId,
        disponible: false,
        status: 'ERROR',
        mensaje: err instanceof Error ? err.message : 'Error al conectar con Homiya',
      };
    }
  }
  ...
  async crearReservaHotel(data: CrearReservaHotelDto): Promise<ReservaHotelExternaDto> {
    try {
      ...
      try {
        ...
        this.logger.warn(`[Homiya] No se pudo obtener precio dinámico, usando fallback.`, err);
      }
      ...
      const res = await this.http.post('/api/v1/mathias-rivera/booking', payload);
      const rawRes = res.data?.data ?? res.data;
      return {
        id: String(rawRes.reservaId ?? rawRes.id ?? 'HMY-' + Date.now()),
      ...
      this.logger.error(`[Homiya] Error creando reserva`, err);
      throw err;
    }
  }
  ...
  async confirmarReservaHotel(id: string): Promise<ReservaHotelExternaDto> {
    try {
      await this.http.patch(`/api/v1/mathias-rivera/booking/${id}/estado`, {
        estado: 'Confirmada',
      });
      return {
        id,
      ...
      this.logger.error(`[Homiya] Error confirmando reserva id=${id}`, err);
      throw err;
    }
  }
  ...
  async cancelarReservaHotel(id: string, reason?: string): Promise<ReservaHotelExternaDto> {
    try {
      ...
      this.logger.error(`[Homiya] Error cancelando reserva id=${id}`, err);
      throw err;
    }
  }
}
